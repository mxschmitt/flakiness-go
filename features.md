...
| 6 | Per-attempt reporting (retries) | ✅ | Re-runs of the same test each emit their own `RunAttempt` with independent status/duration/errors — whether from `-count=N` or the built-in `--rerun-failed[=N]` (rerun only failed tests; fail-then-pass surfaces as flaky while the job stays green). |
