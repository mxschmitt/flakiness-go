...
| 5 | Test hierarchy / suites | ✅ | Tests are grouped into `file` suites titled by their source file (e.g. `pkg/foo_test.go`, with a file location), resolved via `go/ast` — so tests appear under their actual file rather than one import-path bucket. (Tests whose source file can't be resolved fall back to a package-titled `file` suite.) Subtests (`TestX/a/b`) → nested `suite` nodes per path segment; leaves are tests. A parent test that itself fails/panics (not just its subtests) keeps its own attempt as a leaf test inside its suite, so its error is not lost. |
...
| 20 | Errors support | ✅ | Failures are captured as a `ReportError` whose message is the test's whole failure text — multi-line assertions (testify's `Error Trace` / expected-vs-actual / diff) are kept intact — with go test's `=== RUN` / `--- FAIL` framing and per-nesting-level indentation stripped. A panic (or `-timeout` dump) is split into headline (`message`) and goroutine trace (`stack`). Go surfaces a single aggregate failure per test, so one error per attempt. |
